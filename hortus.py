...
def print_results(future_price, data_result, highest_price):
    ...
    rmse = np.sqrt( np.mean(future_price - data_result)**2 )
    ...
    # plt.show()
    plt.savefig('plots/temp/'+Variables.company_name[company_enum])
    plt.clf()
    print('RMSE for', Variables.company_name[company_enum], 'equals', rmse)
